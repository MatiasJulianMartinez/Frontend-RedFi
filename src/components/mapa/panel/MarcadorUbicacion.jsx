...
const MarcadorUbicacion = ({ coordenadas, zona, esValida }) => {
  ...
  return (
    <div className="absolute z-20 transform -translate-x-1/2 -translate-y-full">
      <div
        className={`relative ${esValida ? "text-green-700" : "text-red-500"}`}
      >
        {/* Marcador principal */}
        <div
        ...
        {zona && (
          <div
            className={`absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-1 rounded-lg text-xs font-medium text-white shadow-lg ${
              esValida ? "bg-green-500" : "bg-red-500"
            }`}
          >
            {zona.departamento}
