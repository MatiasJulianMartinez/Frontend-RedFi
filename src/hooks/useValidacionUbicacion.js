...
export const useValidacionUbicacion = (boundsCorrientes) => {
  ...
  const obtenerProveedoresDeZona = useCallback(async (zonaId) => {
    ...
    try {
      ...
      const proveedores = await obtenerProveedoresPorZona(
        zonaId,
   ...
   */
  const validarUbicacion = useCallback(
    async (coordenadas, suprimirMensaje = false) => {
      ...
      try {
        ...
        if (!zona) {
          mostrarErrorRef.current(
            "Esta ubicación no está dentro de ninguna zona con cobertura de internet."
          );
          setUbicacionValida(false);
          setZonaActual(null);
          setProveedoresDisponibles([]);
